// 四川机电职业技术学院(zhjw.scemi.com) 拾光课程表适配脚本
// 基于正方新一代教务系统接口适配
...
//
// 使用方式：校内网络下从校内门户(192.168.73.155/personal-center)登录后进入教务，
// 在教务系统任意页面（如首页、移动端课表页）直接执行导入即可，无需进入课表查询页面。
// 脚本通过接口自动读取学年学期并获取课表与节次作息。
//
// 注意：该校教务仅在校园网内开放：校内门户为 192.168.73.155，教务服务为 192.168.73.166（/jwglxt），
// 公网域名 zhjw.scemi.com 的请求会被 webproxy.scemi.com 按来源网段拦截（403）。
// 脚本会先在当前页面所在主机上尝试正方接口，未命中时回退到校内教务主机，因此从门户或课表页执行都可以。

(function () {
    // 正方新一代教务部分学校接口模块号可能不同，依次尝试常见模块号
    const GNMKDMS = ["N2151", "N253508"];

    // 校内门户与教务服务为两台校内主机，接口地址按候选顺序尝试
    ...
    async function promptUserToStart() {
        return await window.shiguangBridgePromise.showAlert(
            "四川机电职业技术学院课表导入",
            "导入前请确认已在校园网内通过校内门户(192.168.73.155/personal-center)登录并进入教务系统；在任意页面直接执行导入即可，无需进入课表查询页面。",
            "好的，开始导入"
        );
    }
